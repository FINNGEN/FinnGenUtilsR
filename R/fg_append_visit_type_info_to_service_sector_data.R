...
#' @param vocab_omop_concept_relationship full path to the vocabulary omop concep_relationship table for a datafreeze release
#' @param vocab_omop_concept full path to the vocabulary omop concep table for a datafreeze release
#'
...
fg_append_visit_type_info_to_service_sector_data_sql <- function(
  ...
  vocab_omop_concept_relationship |> checkmate::assert_character()
  vocab_omop_concept |> checkmate::assert_character()
  ...
  sql <- system.file("sql/append_visit_type_info_to_service_sector_data.sql", package = "FinnGenUtilsR") |>
    ...
    SqlRender::render(
      ...
      vocab_omop_concept_relationship = vocab_omop_concept_relationship,
      vocab_omop_concept = vocab_omop_concept,
      #
...
#' @param vocab_omop_concept_relationship string with the full path (schema.table) to the database table with concept_relationship
#' @param vocab_omop_concept string with the full path (schema.table) to the database table with concept
...
#' @param dbplyr_vocab_omop_concept_relationship string with the full path (schema.table) to the database table with concept_relationship
#' @param dbplyr_vocab_omop_concept string with the full path (schema.table) to the database table with the concept
...
fg_dbplyr_append_visit_type_info_to_service_sector_data <- function(
    ...
    dbplyr_vocab_omop_concept_relationship,
    dbplyr_vocab_omop_concept,
    ...) {
  # validate
  ...
  sql <- fg_append_visit_type_info_to_service_sector_data_sql(
    ...
    vocab_omop_concept_relationship = paste0( "( ", as.character(dbplyr::sql_render(dbplyr_vocab_omop_concept_relationship)), ")"),
    vocab_omop_concept = paste0( "( ", as.character(dbplyr::sql_render(dbplyr_vocab_omop_concept)), ")"),
    ...
  )
