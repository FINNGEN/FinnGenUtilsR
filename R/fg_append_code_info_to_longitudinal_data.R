...
fg_append_code_info_to_longitudinal_data_sql <- function(
  ...
  ICD10fi_map_to |>
    checkmate::assert_subset(
      c("CODE1_CODE2", "CODE1", "CODE2", "ATC"),
      empty.ok = FALSE
    )
  PURCH_map_to |> checkmate::assert_character(len = 1)
  PURCH_map_to |>
    checkmate::assert_subset(c("ATC", "VNR", "REIMB"), empty.ok = FALSE)
  CANC_map_to |> checkmate::assert_character(len = 1)
  CANC_map_to |>
    checkmate::assert_subset(
      c("MORPO_BEH_TOPO", "TOPO", "MORPHO_BEH"),
      empty.ok = FALSE
    )
  ...
  sql <- system.file(
    "sql/append_info_to_longitudinal_data.sql",
...
fg_bq_append_code_info_to_longitudinal_data <- function(
  ...
  sql <- fg_append_code_info_to_longitudinal_data_sql(
    longitudinal_data_table = paste0(
      bq_table$project,
      ".",
      bq_table$dataset,
      ".",
...
#' @param fg_bq_tables an object of type <fg_bq_tables> with the connections to the bigquery tables. The function will use the connection to the bigquery project and the path to fg_codes_info_table to run the query generated by `fg_append_code_info_to_longitudinal_data_sql`
#' @param ... see [fg_append_code_info_to_longitudinal_data_sql](fg_append_code_info_to_longitudinal_data_sql) for the mapping options
#'
#' @return <tbl> with added columns
#'
#' @importFrom checkmate assert_class
#' @importFrom dbplyr sql_render build_sql sql in_schema
#' @importFrom dplyr tbl
#' @importFrom stringr str_replace_all str_split str_detect str_trim
...
fg_dbplyr_append_code_info_to_longitudinal_data <- function(
  ...
  bq_result <- fg_bq_append_code_info_to_longitudinal_data(
