...
CDMconnectionHandler <- HadesExtras::createCDMdbHandlerFromList(cdm_config)
```
...
CDMconnectionHandler$connectionStatusLog
```
...
Notice that a visit may contain more than one event/row. Events/rows belonging to the same visit share the same combination of SOURCE+INDEX. Additionally we have added diagnostic_relevance column which will produce whether a visit is diagnostic or non-diagnositc visit. For diagnostic_relevance column we need vocabulary tables concept and concept_relationship.
...
fg_dbplyr_append_visit_type_info_to_service_sector_data(
  ss_subject_1_tbl, 
  FGconnectionHandler$getTblmedicalCodesSchema$fg_codes_info_v6(),
  CDMconnectionHandler$getTblVocabularySchema$concept_relationship(),
  CDMconnectionHandler$getTblVocabularySchema$concept()
  )|> 
