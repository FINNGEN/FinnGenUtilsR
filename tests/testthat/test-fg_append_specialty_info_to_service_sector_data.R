...
sql <- paste0("
    SELECT * FROM (
      SELECT *,
        ROW_NUMBER() OVER (PARTITION BY `SOURCE`, `CODE6`) AS q04
...
# TEST
#
test_that("fg_bq_append_specialtiy_info_to_service_sector_data_sql works", {

  skip_if(!bigrquery::bq_table_exists(tb_servicesector_combinations))

  tb_with_translations <- fg_bq_append_specialtiy_info_to_service_sector_data(project_id, tb_servicesector_combinations, fg_codes_info_table)
...
test_that("fg_bq_append_specialtiy_info_to_service_sector_data_sql maps hilmo CODE6", {
  ...
  test_table <- tibble::tibble(
    ...
    CODE6 = c(NA,  NA, NA,  NA, "77", "77", "77", "77"),
  ...
  # DEFAULT
  tb_with_translations <- fg_bq_append_specialtiy_info_to_service_sector_data(
  ...
  res |>
    ...
    dplyr::select(SOURCE, FG_CODE6, speciality_concept_class_id, speciality_code  ) |>
    dplyr::select(SOURCE, speciality_concept_class_id, speciality_code  ) |>
    expect_equal(
      tibble::tibble(
        SOURCE = c('INPAT','OUTPAT','OPER_IN', 'OPER_OUT', 'INPAT','OUTPAT','OPER_IN', 'OPER_OUT'),
        speciality_concept_class_id  = c(rep(as.character(NA), 4),rep("MEDSPECfi Level 0", 4)),
        speciality_code = c(NA,  NA, NA,  NA, "77", "77", "77", "77")
      )
    )
